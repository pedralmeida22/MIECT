...
public class Harbour {
  ...
  HashTable<String> cities = new HashTable<String>(100); 
  ...
  public void enterContainer(int dockId,Container c) {
    ...
    if(cities.contains(c.destination)){
      if(Number.isInteger(cities.get(c.destination))){
       int a = cities.get(c.destination) + 1;
      }
      cities.set(c.destination, a);
    }else{
      cities.set(c.destination, Integer.parseInt(1));
    }
