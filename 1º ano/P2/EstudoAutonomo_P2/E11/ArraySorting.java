...
public class ArraySorting
{
  ...
  static void mergeSort(int[] a, int start, int end)
  {
    ...
    {
      int meio = start + n/2;     // acha o meio do array;
      mergeSort(a, start, meio);  // ordena a primeira metade;
        mergeSort(a, meio+1, end);  // ordena a segunda metade;
      merge(a, start, meio, end); // e funde as duas metades ordenadas.
   ...
   * @param a[start..meio[  A primeira parte ordenada.
   * @param a[meio..end[     A segunda parte ordenada.
  ...
  static void merge(int[] a, int start, int meio, int end)
  {
    ...
    int i1=start, i2=meio;   // índices para percorrer cada parte
    int k = 0;               // índice para percorrer o resultado

    while (i1<meio && i2<end)// Enquanto houver elementos em ambas as partes,
      {
    ...
    while (i1<meio)// Se (enquanto) ainda houver elementos na primeira parte, 
    {
        tmp[k] = a[i1];  // junta-os ao resultado.
    }
    while (i2<end) // Se (enquanto) ainda houver elementos na segunda parte, 
    {
        tmp[k] = a[i2];  // junta-os ao resultado.
    }
  ...
  static void inserePorOrdem(int valor, int[] a, int start, int end)
  {
    ...
    deslocarUm(a, k, end);  // arranja uma "vaga" em a[k]; 
    a[k] = valor;           // coloca o valor no sítio.
  }

  /**
   * Desloca um bloco a[k,end[ para a[k+1,end+1[.
   */
