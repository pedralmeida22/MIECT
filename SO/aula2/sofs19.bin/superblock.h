...
#define TAIL_CACHE_SIZE 170
    ...
    struct SOSuperBlock
    {
        ...
        /** \brief number of head data block */
        uint32_t head_blk;

        /** \brief first occupied position in head data block */
        uint32_t head_idx;

        /** \brief number of tail data block */
        uint32_t tail_blk;

        /** \brief first empty position in tail data block */
        uint32_t tail_idx;
